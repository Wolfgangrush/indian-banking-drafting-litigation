# GEMINI.md — Indian Banking and Financial Litigation Drafting (Gemini CLI port)
...
**Plugin:** `indian-banking-drafting` · **Forum:** Debts Recovery Tribunal bench / Debts Recovery Appellate Tribunal / National Company Law Tribunal bench / Chief Metropolitan Magistrate / District Magistrate / civil court for SARFAESI · **Publisher:** wolfgang_rush · **License:** MIT

---

## 🇮🇳 WHO YOU ARE TALKING TO

The user is an **enrolled Indian advocate** drafting banking and financial litigation pleadings for filing. They are running the 6-agent drafting pipeline (reader → format → drafter → verifier → refiner → overseer) and expect filing-grade output (.docx) with anti-hallucination discipline.
...
Every pipeline run reads the user's `case-config.md` from the case folder. Substitute every `{{config.X}}` placeholder in the format-shell with the real value before passing to the Drafter.
...
- Preserve the advocate's bench's idiom (substituted from case-config.md)

### Rule 8 — STOP on missing inputs
- Reader STOPS if required law PDF missing
- Format STOPS if case-config.md missing or incomplete
...
```
<case-folder>/
├── case-config.md    ← required, advocate-supplied
...
Original Application (OA) under RDB Act 1993 · SARFAESI Section 17 application · DRAT appeal · IBC Section 7 / 9 / 10 · Section 138 NI Act complaint · MSMED Act delayed payment · Section 14 SARFAESI possession application · Section 13(2) SARFAESI demand notice reply · Section 19 RDB Act counter-claim

Each case type has its own template in `skills/ (banking case-type templates)`.
